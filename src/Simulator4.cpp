...
std::vector<OddsResult> Simulator4::get_probability(std::string hero_name, const std::vector<std::string> & abilities, std::string diceanatomy, bool default_sim, bool chase, std::string chased_ability, std::vector<DiceIdx> org_dice, size_t sixit, size_t samesis, size_t tip_it, size_t wild, size_t twiceWild, size_t slightlyWild, size_t cp, size_t use_max_cards, size_t roll_atemps, size_t rerolls)
{
            ...
            for (size_t anzcards = 0; anzcards <= max_cards; anzcards++)
            {
                if (start_number_lines <= done_counter_anz)
                {
                    ...
                    if (tmili > print_time)
                    {
                        ...
                        std::cout << ability_name << " " << ss.str() << " sim 4 preparation took " << tmili << " seconds, " << int(10000.0 * proz_done) / 100.0 << "%" << std::endl;
                        std::cout << ability_name << " " << ss.str() << " sim 4 remaining time: " << time_remain << " min, " << time_remain / 60.0 << " h " << std::endl;
                    }
                    size_t counter = 0;
