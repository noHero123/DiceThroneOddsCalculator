...
namespace MCTSDUCTSmallFast
{
	...
	public:
		...
		size_t select_child_DUCT(State& state, RandomEngine* engine, const double& c, Node<State>& newNode, size_t& newNodeID)
		{
			...
			else
			{
				...
				this->lastSelectedMovesPerPlayer = bestMove;
		...
		const std::vector<Node<State>>& nodes, const Node<State>& best_children)
	{
		...
		{
			auto it = best_children.children.find(i);
			if (it == best_children.children.end())
			{
				continue;
			}
			size_t cs_idx = it->second;
			//random childs
			...
			else
			{
				...
				if (best_children.tree_lvl == 1 && max_targt == 5)
				{
					std::cout << max_targt << "\r\n";
				}
				size_t bestmove = css.moves[max_targt];
				auto its = css.children.find(bestmove);
				if (its != css.children.end())
				{
					size_t max_targt_idx = its->second;
					const auto& best_children2 = nodes[max_targt_idx];
		...
		std::vector<MCTSDUCTSmallFast::Node<ORP>>& nodes)
	{
		...
		if (options.verbose)
		{
			...
			auto it = nodes[0].children.find(best_move);
			if (it != nodes[0].children.end())
			{
				size_t itidx = it->second;
				float ergo = get_prob(nodes, nodes[itidx]);
