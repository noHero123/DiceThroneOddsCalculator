...
{
public:
	//special version: treats last die seperately (=scarlett-die)
	typedef size_t Move;
    static const Move no_move = 1048576; // 2^20
	static const size_t num_dice_ = 5;
	static const size_t max_move_s = 31;//15
	ORP4()
	{
	}
	...
	std::vector<DiceHash> get_rerolls(const std::vector<DiceIdx>& dont_reroll, const std::vector<DiceIdx> allways_reroll, const std::vector<DiceIdx>& target, const std::vector<DiceIdx>& anatomy)
	{
		...
		for (const auto& roll_idx : allowed_rerolls)
		{
			...
			if (bits[4])
			{
				size_t diceval_last = (*inverted_dice_anatomy)[dice[4]];
				value += 117649 * (diceval_last + 1);
			}
			bool found = false;
		...
		//FOR TESTING#############################################
		/*std::vector<size_t> allrerolsidx{};
		for (const auto& roll_idx : allowed_rerolls)
		{
			const std::vector<bool>& bits = (*allowed_rerolls_vecs)[roll_idx];
			size_t val = 0;
			for (size_t i = 0; i < num_dice_; i++)
			{
				...
				{
					size_t diceval = (*inverted_dice_anatomy)[dice[i]];
					val = val * 10 + (diceval + 1);
				}
			}
			allrerolsidx.push_back(val);
		}
		std::vector<size_t> allrerolsidx2{};
		for (const auto& roll_idx : allowed_rerolls3)
		{
			const std::vector<bool>& bits = (*allowed_rerolls_vecs)[roll_idx];
			size_t val = 0;
			for (size_t i = 0; i < num_dice_; i++)
	...
	size_t last_move{ max_move_s };
	size_t last_reroll_move{ max_move_s };
